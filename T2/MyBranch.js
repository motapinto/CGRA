...
class MyBranch extends CGFobject {
    constructor(scene, slices, height, radius) {
        ...
        //Position parameters
        this.init_x = -5 + Math.floor((Math.random() * 4) -4);
        this.init_y = radius;
        this.init_z = 10 + Math.floor((Math.random() * 6) + 1);
    ...
    pick() {
        ...
        let z_init = this.scene.tree_branch.z + -3;
        let z_final = this.scene.tree_branch.z + 3;
        let x_init = this.scene.tree_branch.x - 3;
        let x_final = this.scene.tree_branch.x + 3;
